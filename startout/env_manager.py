...
class EnvironmentVariableManager:
    ...
    def capture_final_env(self):
        ...
        new_vars = {key: value for key, value in current_vars.items() if key not in self.initial_vars}
    ...
    def get_captured_vars(self) -> Tuple[dict, dict]:
        ...
        **Returns:**
        ...
        """
        potentially_sensitive = {key: value for key, value in self.final_vars.items() if
                                 is_potentially_sensitive_key_value(key, value)}
        not_potentially_sensitive = {key: value for key, value in self.final_vars.items() if
                                     not is_potentially_sensitive_key_value(key, value)}
