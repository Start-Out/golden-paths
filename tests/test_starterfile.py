...
class Tool:
    def __init__(
        self,
        name,
        mock_check: bool = False,
        mock_initialize: bool = False,
        mock_destroy: bool = False,
    ):
        self.name = name
...
class TestStarter(unittest.TestCase):
    def setUp(self):
        ...
        self.starter_successful_init = Starter(
            self.mock_modules, self.mock_tools, self.module_deps, self.tool_deps
        )
    ...
    def test_get_init_options(self):
        assert self.starter_successful_init.get_init_options() == [
            ("", InitOption(str(i))) for i in range(5)
        ]
...
def test_install_tools_no_tools():
    starter = Starter([], [], [], [])
    assert (
        not starter.install_tools()
    ), "Should return False when there are no tools to install."
...
def test_install_tools_all_tools_installed():
    ...
    assert (
        starter.install_tools()
    ), "Should return True when all tools are already installed."
...
def test_install_tools_all_tools_uninstalled():
    ...
    assert (
        starter.install_tools()
    ), "Should return True when all tools are successfully installed."
...
def test_install_tools_fail_early():
    ...
    starter = Starter([], tools, [], [[t.name for t in tools]])
    assert not starter.install_tools(
        fail_early=True
    ), "Should return False as tool '5' fails to initialize."
...
def test_install_tools_teardown_on_failure():
    ...
    with pytest.raises(SystemExit):
        ...
        ), "Should raise SystemExit as tool '6' fails to destroy."
...
def test_install_modules_no_modules():
    starter = Starter([], [], [], [])
    assert (
        not starter.install_modules()
    ), "Should return False when there are no modules to install."
...
def test_install_modules_all_modules_installed():
    ...
    assert (
        starter.install_modules()
    ), "Should return True when all modules are already installed."
...
def test_install_modules_all_modules_uninstalled():
    ...
    assert (
        starter.install_modules()
    ), "Should return True when all modules are successfully installed."
...
def test_install_modules_fail_early():
    ...
    assert not starter.install_modules(
        fail_early=True
    ), "Should return False as module '5' fails to initialize."
...
def test_install_modules_teardown_on_failure():
    ...
    with pytest.raises(SystemExit):
        starter.install_modules(
            teardown_on_failure=True
        ), "Should raise SystemExit as module '6' fails to destroy."
