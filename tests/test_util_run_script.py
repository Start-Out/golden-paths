...
class TestRunScriptWithEnvSubstitution(unittest.TestCase):
    ...
    def test_run_script_with_env_substitution_single_line_command(
        self, mock_split, mock_which, mock_run
    ):
    ...
    @mock.patch("shutil.which")
    @mock.patch("shlex.split")
    def test_run_script_with_env_substitution_unavailable_command(
        self, mock_split, mock_which, mock_run
    ):
        mock_split.return_value = ["unavailable_command"]
        mock_which.return_value = None
        mock_run.return_value = subprocess.CompletedProcess(
            [], stdout="unavailable_command not found", returncode=1
        )

        output, returncode = util.run_script_with_env_substitution(
    ...
    @mock.patch("shutil.which")
    @mock.patch("shlex.split")
    def test_run_script_with_env_substitution_command_failure(
        self, mock_split, mock_which, mock_run
    ):
        mock_split.return_value = ["failing_command"]
        mock_which.return_value = "/bin/failing_command"
        mock_run.side_effect = subprocess.CalledProcessError(
            1, "failing_command", stderr="An error occurred"
        )
