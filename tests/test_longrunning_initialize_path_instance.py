...
class TestInitializePathInstance(unittest.TestCase):
    ...
    def test_initialize_path_instance_fully_formed(self, mock_re, mock_check, mock_new_owner, mock_init_repo,
                                                   mock_chdir, mock_open, mock_parse, mock_prompt):
        #####################
        # Define interactions
        ...
        mock_parse.side_effect = lambda x: x  # This function will simply return what it received
        mock_prompt.side_effect = lambda x: x  # This function will simply return what it received
        #####################

        _ = startout.paths.initialize_path_instance(self.fully_formed_template_name, self.new_repo_name,
                                                    self.new_repo_owner, self.public)

        mock_re.assert_called_once_with(r"^[^/]*/[^/]*$", self.fully_formed_template_name)

    def test_initialize_path_instance_invalid_template(self, mock_re, mock_check, mock_new_owner, mock_init_repo,
                                                       mock_chdir, mock_open, mock_parse, mock_prompt):
        #####################
        # Define interactions
        ...
        with self.assertRaises(SystemExit):
            startout.paths.initialize_path_instance(self.fully_formed_template_name, self.new_repo_name,
                                                    self.new_repo_owner, self.public)

    def test_initialize_path_instance_startout_valid_path(self, mock_re, mock_check, mock_new_owner, mock_init_repo,
                                                          mock_chdir, mock_open, mock_parse, mock_prompt):
        #####################
        # Define interactions
    ...
    def test_initialize_path_instance_non_startout_non_path(self, mock_re, mock_check, mock_new_owner,
                                                            mock_init_repo, mock_chdir, mock_open, mock_parse,
                                                            mock_prompt):
        #####################
        # Define interactions
        self.mock_console.input.side_effect = ["Not-Start-Out"]  # Give a Non-StartOut owner
        mock_re.return_value = False  # The template defined is not a valid reference
    ...
    def test_initialize_path_with_interactive_new_owner(self, mock_re, mock_check, mock_new_owner, mock_init_repo,
                                                        mock_chdir, mock_open, mock_parse, mock_prompt):
        #####################
        # Define interactions
        mock_re.return_value = True
        ...
        mock_check.mock_new_owner = "goose"  # Simulate interactively prompting for the owner if not defined
        #####################
    ...
    def test_initialize_path_init_repo_fails(self, mock_re, mock_check, mock_new_owner, mock_init_repo, mock_chdir,
